...
void Terrain3D::_initialize() {
	...
	if (!_data->is_connected("maps_edited", callable_mp(_instancer, &Terrain3DInstancer::update_transforms))) {
		print_line(DEBUG, "Connecting maps_edited signal to update_transforms()");
...
void Terrain3D::set_texture_list(const Ref<Terrain3DTextureList> &p_texture_list) {
	if (p_texture_list.is_null()) {
		print_line(ERROR, "Attempted to upgrade Terrain3DTextureList, but received null (perhaps already a Terrain3DAssets). Reconnect manually and save.");
		return;
	}
	print_line(WARN, "Loaded Terrain3DTextureList. Converting to Terrain3DAssets. Save this scene to complete.");
...
void Terrain3D::set_storage(const Ref<Terrain3DStorage> &p_storage) {
	...
	if (p_storage.is_valid()) {
		print_line(WARN, "Loaded Terrain3DStorage v", vformat("%.3f", p_storage->get_version()), ". Use Terrain3D Tools / Directory Setup to upgrade");
...
void Terrain3D::split_storage() {
	if (_storage.is_null()) {
		print_line(ERROR, "Storage has not been loaded");
		return;
	}
	if (_data == nullptr) {
		print_line(ERROR, "_data has not been initialized");
		return;
	}
	if (_data_directory.is_empty()) {
		print_line(ERROR, "Data_directory is empty");
		return;
	}
	...
	for (int i = 0; i < locations.size(); i++) {
		...
		print_line(INFO, "Splicing region ", locations[i]);
	}
	_storage.unref();
	_data->force_update_maps();
	_instancer->force_update_mmis();
	print_line(WARN, "Terrain3DStorage has been converted to Terrain3DData. Save to write changes to disk");
}
