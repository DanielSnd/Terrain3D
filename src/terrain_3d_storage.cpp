...
void Terrain3DStorage::set_version(const real_t p_version) {
	...
	if (_version < CURRENT_VERSION) {
		TERRAINLOG(WARN, "Storage version ", vformat("%.3f", _version), " will be updated to ", vformat("%.3f", CURRENT_VERSION), " upon save");
	}
}
...
void Terrain3DStorage::set_region_offsets(const TypedArray<Vector2i> &p_offsets) {
	TERRAINLOG(INFO, "Setting region offsets with array sized: ", p_offsets.size());
...
void Terrain3DStorage::set_maps(const MapType p_map_type, const TypedArray<Image> &p_maps) {
	...
	TERRAINLOG(INFO, "Setting ", TYPESTR[p_map_type], " maps: ", p_maps.size());
	switch (p_map_type) {
		case TYPE_HEIGHT:
			_height_maps = p_maps;
			break;
...
void Terrain3DStorage::set_multimeshes(const Dictionary &p_multimeshes) {
	TERRAINLOG(INFO, "Loading multimeshes: ", p_multimeshes);
...
void Terrain3DStorage::_bind_methods() {
	...
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "region_offsets", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s", Variant::VECTOR2, PROPERTY_HINT_NONE), ro_flags), "set_region_offsets", "get_region_offsets");
