...
{
        if (_rid.is_valid()) {
		TERRAINLOG(EXTREME, "GeneratedTexture freeing ", _rid);
		RenderingServer::get_singleton()->free(_rid);
	}
	if (_image.is_valid()) {
		TERRAINLOG(EXTREME, "GeneratedTexture unref image", _image);
		_image.unref();
	}
	_rid = RID();
	_dirty = true;
...
RID GeneratedTexture::create(const TypedArray<Image> &p_layers) {
	...
	if (!p_layers.is_empty()) {
		if (Terrain3D::debug_level >= DEBUG) {
			...
			for (int i = 0; i < p_layers.size(); i++) {
				Ref<Image> img = p_layers[i];
				TERRAINLOG(EXTREME, i, ": ", img, ", empty: ", img->is_empty(), ", size: ", img->get_size(), ", format: ", img->get_format());
			}
		}
...
void GeneratedTexture::update(const Ref<Image> &p_image, int p_layer) {
	TERRAINLOG(EXTREME, "RenderingServer updating Texture2DArray at index: ", p_layer);
	RenderingServer::get_singleton()->texture_2d_update(_rid, p_image, p_layer);
}

RID GeneratedTexture::create(const Ref<Image> &p_image) {
	TERRAINLOG(EXTREME, "RenderingServer creating Texture2D");
	_image = p_image;
