...
void Terrain3DData::_clear() {
	print_line(INFO, "Clearing storage");
...
void Terrain3DData::initialize(Terrain3D *p_terrain) {
	if (p_terrain == nullptr) {
		print_line(ERROR, "Initialization failed, p_terrain is null");
		return;
	}
	print_line(INFO, "Initializing storage");
...
void Terrain3DData::set_region_locations(const TypedArray<Vector2i> &p_locations) {
	print_line(INFO, "Setting _region_locations with array sized: ", p_locations.size());
...
void Terrain3DData::do_for_regions(const Rect2i &p_area, const Callable &p_callback) {
	...
	print_line(DEBUG, "Processing global area: ", p_area, " -> ", location_bounds);
	...
	for (int y = location_bounds.position.y; y < location_bounds.get_end().y; y++) {
		...
		for (int x = location_bounds.position.x; x < location_bounds.get_end().x; x++) {
			...
			if (region.is_valid() && !region->is_deleted()) {
				print_line(DEBUG, "Current region: ", current_region_loc);
				Rect2i region_area = p_area.intersection(Rect2i(current_region_loc * _region_size, _region_sizev));
				print_line(DEBUG, "Region bounds: ", Rect2i(current_region_loc * _region_size, _region_sizev));
				print_line(DEBUG, "Region area: ", region_area);
				...
				print_line(DEBUG, "src map coords: ", src_coords);
				print_line(DEBUG, "dst map coords: ", dst_coords);
...
void Terrain3DData::change_region_size(int p_new_size) {
	print_line(INFO, "Changing region size from: ", _region_size, " to ", p_new_size);
	if (p_new_size < 64 || p_new_size > 2048 || !is_power_of_2(p_new_size)) {
		print_line(ERROR, "Invalid region size: ", p_new_size, ". Must be 64, 128, 256, 512, 1024, 2048");
		return;
	}
...
void Terrain3DData::set_region_modified(const Vector2i &p_region_loc, const bool p_modified) {
	...
	if (region.is_null()) {
		print_line(ERROR, "Region not found at: ", p_region_loc);
		return;
	}
...
Error Terrain3DData::add_region(const Ref<Terrain3DRegion> &p_region, const bool p_update) {
	if (p_region.is_null()) {
		print_line(ERROR, "Provided region is null. Returning");
		return FAILED;
	}
	Vector2i region_loc = p_region->get_location();
	print_line(INFO, "Adding region at location ", region_loc, ", update maps: ", p_update ? "yes" : "no");

	// Check bounds and slow report errors
	if (get_region_map_index(region_loc) < 0) {
		print_line(ERROR, "Location ", region_loc, " out of bounds. Max: ",
	...
	if (!_region_locations.has(region_loc)) {
		...
		print_line(INFO, "Overwriting ", (_regions.has(region_loc)) ? "deleted" : "existing", " region at ", region_loc);
	}
	_regions[region_loc] = p_region;
	_region_map_dirty = true;
	print_line(DEBUG, "Storing region ", region_loc, " version ", vformat("%.3f", p_region->get_version()), " id: ", _region_locations.size());
	if (p_update) {
		force_update_maps();
	}
	return OK;
...
void Terrain3DData::remove_region(const Ref<Terrain3DRegion> &p_region, const bool p_update) {
	if (p_region.is_null()) {
		print_line(ERROR, "Region not found or is null. Returning");
		return;
	}

	Vector2i region_loc = p_region->get_location();
	int region_id = _region_locations.find(region_loc);
	print_line(INFO, "Marking region ", region_loc, " for deletion. update_maps: ", p_update ? "yes" : "no");
	if (region_id < 0) {
		print_line(ERROR, "Region ", region_loc, " not found in region_locations. Returning");
		return;
	}
	p_region->set_deleted(true);
	_region_locations.remove_at(region_id);
	_region_map_dirty = true;
	print_line(DEBUG, "Removing from region_locations, new size: ", _region_locations.size());
	if (p_update) {
		print_line(DEBUG, "Updating generated maps");
		force_update_maps();
	}
}

void Terrain3DData::save_directory(const String &p_dir) {
	print_line(INFO, "Saving data files to ", p_dir);
...
void Terrain3DData::save_region(const Vector2i &p_region_loc, const String &p_dir, const bool p_16_bit) {
	Ref<Terrain3DRegion> region = get_region(p_region_loc);
	if (region.is_null()) {
		print_line(ERROR, "No region found at: ", p_region_loc);
		return;
	}
	...
	if (region->is_deleted()) {
		print_line(DEBUG, "Removing ", p_region_loc, " from _regions");
		_regions.erase(p_region_loc);
		print_line(DEBUG, "File to be deleted: ", path);
		if (!FileAccess::exists(path)) {
			print_line(INFO, "File to delete ", path, " doesn't exist. (Maybe from add, undo, save)");
			return;
		}
		Ref<DirAccess> da = DirAccess::open(p_dir);
		if (da.is_null()) {
			print_line(ERROR, "Cannot open directory for writing: ", p_dir, " error: ", DirAccess::get_open_error());
			return;
		}
		Error err = da->remove(fname);
		if (err != OK) {
			print_line(ERROR, "Could not remove file: ", fname, ", error code: ", err);
		}
		print_line(INFO, "File ", path, " deleted");
		return;
	}
	Error err = region->save(path, p_16_bit);
	if (!(err == OK || err == ERR_SKIP)) {
		print_line(ERROR, "Could not save file: ", path, ", error: ", err, " (", err, ")");
	}
}

void Terrain3DData::load_directory(const String &p_dir) {
	if (p_dir.is_empty()) {
		print_line(ERROR, "Specified data directory is blank");
		return;
	}
	Ref<DirAccess> da = DirAccess::open(p_dir);
	if (da.is_null()) {
		print_line(ERROR, "Cannot read Terrain3D data directory: ", p_dir);
		return;
	}
	_clear();

	print_line(INFO, "Loading region files from ", p_dir);
	...
	for (int i = 0; i < files.size(); i++) {
		...
		print_line(DEBUG, "Loading region from ", path);
		Vector2i loc = Util::filename_to_location(fname);
		if (loc.x == INT32_MAX) {
			print_line(ERROR, "Cannot get region location from file name: ", fname);
			continue;
		}
		Ref<Terrain3DRegion> region = ResourceLoader::load(path, "Terrain3DRegion", ResourceFormatLoader::CacheMode::CACHE_MODE_IGNORE);
		if (region.is_null()) {
			print_line(ERROR, "Cannot load region at ", path);
			continue;
		}
		print_line(INFO, "Loaded region: ", loc, " size: ", region->get_region_size());
		if (_regions.is_empty()) {
			...
			if (_terrain->get_region_size() != (Terrain3D::RegionSize)region->get_region_size()) {
				print_line(ERROR, "Region size mismatch. First loaded: ", _terrain->get_region_size(), " next: ",
						region->get_region_size(), " in file: ", path);
				return;
			}
		}
		region->call("take_over_path",path);
...
void Terrain3DData::load_region(const Vector2i &p_region_loc, const String &p_dir, const bool p_update) {
	print_line(INFO, "Loading region from location ", p_region_loc);
	String path = p_dir + String("/") + Util::location_to_filename(p_region_loc);
	if (!FileAccess::exists(path)) {
		print_line(ERROR, "File ", path, " doesn't exist");
		return;
	}
	Ref<Terrain3DRegion> region = ResourceLoader::load(path, "Terrain3DRegion", ResourceFormatLoader::CacheMode::CACHE_MODE_IGNORE);
	if (region.is_null()) {
...
TypedArray<Image> Terrain3DData::get_maps(const MapType p_map_type) const {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
		print_line(ERROR, "Specified map type out of range");
...
void Terrain3DData::force_update_maps(const MapType p_map_type, const bool p_generate_mipmaps) {
	print_line(EXTREME, "Regenerating maps of type: ", p_map_type);
	switch (p_map_type) {
		case TYPE_HEIGHT:
			_generated_height_maps.clear();
			break;
	...
	if (p_generate_mipmaps && (p_map_type == TYPE_COLOR || p_map_type == TYPE_MAX)) {
		print_line(EXTREME, "Regenerating color mipmaps");
		for (int i = 0; i < _region_locations.size(); i++) {
			Vector2i region_loc = _region_locations[i];
			Ref<Terrain3DRegion> region = get_region(region_loc);
...
void Terrain3DData::update_maps(const MapType p_map_type) {
	...
	if (_region_map_dirty) {
		print_line(EXTREME, "Regenerating ", REGION_MAP_VSIZE, " region map array from active regions");
		_region_map.clear();
		_region_map.resize(REGION_MAP_SIZE * REGION_MAP_SIZE);
		...
		for (int i = 0; i < locs.size(); i++) {
			...
			if (region.is_valid() && !region->is_deleted()) {
				...
				if (map_index >= 0) {
					_region_map.write[map_index] = region_id;
	...
	if (_generated_height_maps.is_dirty()) {
		print_line(EXTREME, "Regenerating height texture array from regions");
		_height_maps.clear();
		for (int i = 0; i < _region_locations.size(); i++) {
			Vector2i region_loc = _region_locations[i];
			Ref<Terrain3DRegion> region = get_region(region_loc);
			if (region.is_valid()) {
				...
				print_line(ERROR, "Can't find region ", region_loc, ", _regions: ", _regions,
	...
	if (_generated_control_maps.is_dirty()) {
		print_line(EXTREME, "Regenerating control texture array from regions");
		_control_maps.clear();
		for (int i = 0; i < _region_locations.size(); i++) {
			Vector2i region_loc = _region_locations[i];
			Ref<Terrain3DRegion> region = get_region(region_loc);
	...
	if (_generated_color_maps.is_dirty()) {
		print_line(EXTREME, "Regenerating color texture array from regions");
		_color_maps.clear();
		for (int i = 0; i < _region_locations.size(); i++) {
			Vector2i region_loc = _region_locations[i];
			Ref<Terrain3DRegion> region = get_region(region_loc);
...
void Terrain3DData::set_pixel(const MapType p_map_type, const Vector3 &p_global_position, const Color &p_pixel) {
	...
	Ref<Terrain3DRegion> region = get_region(region_loc);
	if (region.is_null()) {
		print_line(ERROR, "No region found at: ", p_global_position);
		return;
	}
...
Vector3 Terrain3DData::get_mesh_vertex(const int32_t p_lod, const HeightFilter p_filter, const Vector3 &p_global_position) const {
	print_line(INFO, "Calculating vertex location");
...
void Terrain3DData::calc_height_range(const bool p_recursive) {
	...
	for (int i = 0; i < _region_locations.size(); i++) {
		...
		Ref<Terrain3DRegion> region = get_region(region_loc);
		if (region.is_null()) {
			print_line(ERROR, "Region not found at: ", region_loc);
			return;
		}
		if (p_recursive) {
	...
	print_line(EXTREME, "Accumulated height range for all regions: ", _master_height_range);
}
...
void Terrain3DData::import_images(const TypedArray<Image> &p_images, const Vector3 &p_global_position, const real_t p_offset, const real_t p_scale) {
	...
	if (p_images.size() != TYPE_MAX) {
		print_line(ERROR, "p_images.size() is ", p_images.size(), ". It should be ", TYPE_MAX, " even if some Images are blank or null");
		return;
	}
	...
	for (int i = 0; i < TYPE_MAX; i++) {
		...
		if (img.is_valid() && !img->is_empty()) {
			print_line(INFO, "Importing image type ", TYPESTR[i], ", size: ", img->get_size(), ", format: ", img->get_format());
			if (i == TYPE_HEIGHT) {
				print_line(INFO, "Applying offset: ", p_offset, ", scale: ", p_scale);
			}
			if (img_size == V2I_ZERO) {
				img_size = img->get_size();
			} else if (img_size != img->get_size()) {
				print_line(ERROR, "Included Images in p_images have different dimensions. Aborting import");
				return;
			}
		}
	}
	if (img_size == V2I_ZERO) {
		print_line(ERROR, "All images are empty. Nothing to import");
		return;
	}
	...
	if ((abs(descaled_position.x) > max_dimension) || (abs(descaled_position.z) > max_dimension)) {
		print_line(ERROR, "Specify a position within +/-", Vector3(max_dimension, 0.f, max_dimension) * _vertex_spacing);
		return;
	}
	if ((descaled_position.x + img_size.x > max_dimension) ||
			(descaled_position.z + img_size.y > max_dimension)) {
		print_line(ERROR, img_size, " image will not fit at ", p_global_position,
	...
	for (int i = 0; i < TYPE_MAX; i++) {
		...
		if (i == TYPE_HEIGHT && (p_offset != 0.f || p_scale != 1.f)) {
			print_line(DEBUG, "Creating new temp image to adjust scale: ", p_scale, " offset: ", p_offset);
	...
	print_line(DEBUG, "Creating ", Vector2i(slices_width, slices_height), " slices for ", img_size, " images.");
	...
	for (int y = 0; y < slices_height; y++) {
		for (int x = 0; x < slices_width; x++) {
			...
			print_line(DEBUG, "Reviewing image section ", start_coords, " to ", end_coords);
			...
			if (end_coords.x <= img_size.x && end_coords.y <= img_size.y) {
				...
				print_line(DEBUG, "Uneven end piece. Copying padded slice ", Vector2i(x, y), " size to copy: ", size_to_copy);
			}

			print_line(DEBUG, "Copying ", size_to_copy, " sized segment");
...
Error Terrain3DData::export_image(const String &p_file_name, const MapType p_map_type) const {
	if (p_map_type < 0 || p_map_type >= TYPE_MAX) {
		print_line(ERROR, "Invalid map type specified: ", p_map_type, " max: ", TYPE_MAX - 1);
		return FAILED;
	}

	if (p_file_name.is_empty()) {
		print_line(ERROR, "No file specified. Nothing to export");
		return FAILED;
	}

	if (get_region_count() == 0) {
		print_line(ERROR, "No valid regions. Nothing to export");
		return FAILED;
	}
	...
	for (int i = 0; i < bad_chars.length(); ++i) {
		for (int j = 0; j < p_file_name.length(); ++j) {
			if (bad_chars[i] == p_file_name[j]) {
				print_line(ERROR, "Invalid file path '" + p_file_name + "'");
				return FAILED;
			}
		}
	}
	...
	if (file_ref.is_null()) {
		print_line(ERROR, "Cannot open file '" + file_name + "' for writing");
		return FAILED;
	}
	file_ref->close();
	...
	if (img.is_null() || img->is_empty()) {
		print_line(ERROR, "Cannot create an export image for map type: ", TYPESTR[p_map_type]);
		return FAILED;
	}

	String ext = file_name.get_extension().to_lower();
	print_line(MESG, "Saving ", img->get_size(), " sized ", TYPESTR[p_map_type],
	...
	print_line(ERROR, "No recognized file type. See docs for valid extensions");
	return FAILED;
}

Ref<Image> Terrain3DData::layered_to_image(const MapType p_map_type) const {
	print_line(INFO, "Generating a full sized image for all regions including empty regions");
	...
	for (int i = 0; i < _region_locations.size(); i++) {
		print_line(DEBUG, "Region locations[", i, "]: ", _region_locations[i]);
		Vector2i region_loc = _region_locations[i];
	...
	print_line(DEBUG, "Full range to cover all regions: ", top_left, " to ", bottom_right);
	Vector2i img_size = Vector2i(1 + bottom_right.x - top_left.x, 1 + bottom_right.y - top_left.y) * _region_size;
	print_line(DEBUG, "Image size: ", img_size);
	...
	for (int i = 0; i < _region_locations.size(); i++) {
		...
		print_line(DEBUG, "Region to blit: ", region_loc, " Export image coords: ", img_location);
		Ref<Terrain3DRegion> region = get_region(region_loc);
...
void Terrain3DData::print_audit_data() const {
	print_line(INFO, "Dumping storage data");
	print_line(INFO, "Region_locations size: ", _region_locations.size(), " ", _region_locations);
	print_line(INFO, "Region map");
	for (int i = 0; i < _region_map.size(); i++) {
		if (_region_map[i]) {
			print_line(INFO, "Region id: ", _region_map[i], " array index: ", i);
		}
	}
...
void Terrain3DData::_bind_methods() {
	...
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "region_locations", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s", Variant::VECTOR2, PROPERTY_HINT_NONE), ro_flags), "set_region_locations", "get_region_locations");
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "height_maps", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s:%s", Variant::OBJECT, PROPERTY_HINT_RESOURCE_TYPE, "Image"), ro_flags), "", "get_height_maps");
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "control_maps", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s:%s", Variant::OBJECT, PROPERTY_HINT_RESOURCE_TYPE, "Image"), ro_flags), "", "get_control_maps");
	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "color_maps", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s:%s", Variant::OBJECT, PROPERTY_HINT_RESOURCE_TYPE, "Image"), ro_flags), "", "get_color_maps");
